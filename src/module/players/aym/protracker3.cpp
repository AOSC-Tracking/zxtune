  ...
  namespace TS
  {
    ...
    public:
      ...
      Devices::TurboSound::Registers GetData() const override
      {
        ...
        auto state = *Delegate->GetState().Track;
        auto firstReg = RenderFrom(state, *First);
        state.Pattern = Base - 1 - state.Pattern;
        return {{std::move(firstReg), RenderFrom(state, *Second)}};
      }
    ...
    {
    public:
      Chiptune(uint_t base, ModuleData::Ptr data, Parameters::Accessor::Ptr properties)
        : Base(base)
        , Data(std::move(data))
        , Properties(std::move(properties))
      {}
      ...
      {
        auto iterator = CreateTrackStateIterator(GetFrameDuration(), Data);
        return MakePtr<DataIterator>(Base, std::move(first), std::move(iterator), Vortex::CreateDataRenderer(Data),
                                     Vortex::CreateDataRenderer(Data));
      }
                             ...
                             Parameters::Container::Ptr properties) const override
    {
      ...
      if (const auto container = Decoder->Parse(rawData, dataBuilder))
      {
        ...
        if (patOffset != Formats::Chiptune::ProTracker3::SINGLE_AY_MODE)
        {
          ...
          auto chiptune = MakePtr<TS::Chiptune>(patOffset, std::move(modData), std::move(properties));
