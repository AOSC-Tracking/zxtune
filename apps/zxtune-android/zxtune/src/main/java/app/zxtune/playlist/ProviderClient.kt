...
class ProviderClient @VisibleForTesting constructor(
    ...
    private val playlist: Playlist.Id,
    private val dispatcher: CoroutineDispatcher,
) {
    private val playlistUri = Query.localPlaylistUri(playlist)
    ...
    fun observeContent() = resolver.observeChanges(playlistUri).debounce(1000).transform {
        queryContent()?.let {
            emit(it)
        }
    }
    ...
    suspend fun statistics(tracks: Track.IdSet?) = withContext(dispatcher) {
        ...
        Provider.statistics(resolver, Playlist.OperationScope(playlist, tracks))?.asStatistics()
    }

    @Throws(Exception::class)
    suspend fun save(name: String, tracks: Track.IdSet?) = withContext(dispatcher) {
        Provider.save(resolver, name, Playlist.OperationScope(playlist, tracks))
